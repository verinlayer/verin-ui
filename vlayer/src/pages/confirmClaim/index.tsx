...
import { AlreadyClaimedError } from "../../shared/errors/appErrors";
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {txHash && status === "pending" && (
        <div className="mb-4 p-3 bg-cyan-900/20 border border-cyan-500/50 rounded-lg backdrop-blur-sm">
          <div className="flex items-center">
            <div className="animate-spin rounded-full h-5 w-5 border-b-2 border-cyan-400 mr-2"></div>
            <div>
              <div className="text-sm font-medium text-cyan-400">Transaction broadcasted</div>
      ...
      {userCancelled && (
        <div className="mb-4 p-3 bg-yellow-900/20 border border-yellow-500/50 rounded-lg backdrop-blur-sm">
          <div className="flex items-center">
            <svg className="w-5 h-5 text-yellow-400 mr-2" fill="currentColor" viewBox="0 0 20 20">
              <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd" />
            </svg>
            <div>
              <div className="text-sm font-medium text-yellow-400">Transaction Cancelled</div>
              <div className="text-xs text-yellow-300">You rejected the transaction in your wallet. You can try again when ready.</div>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="mb-4 w-full block">
        ...
        <input
          ...
          className="w-full p-2 border border-slate-600 rounded-md bg-slate-700/50 text-slate-200"
          disabled
        />
      </div>
      <div className="mb-4 w-full block">
